...
// MasterChef is the master of Rio. He can make Rio and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once RIO is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of RIOs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. RIOs to distribute per block.
    ...
    uint256 public rioPerBlock;
    // Bonus muliplier for early rio makers.
    ...
    // Update the given pool's RIO allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingRio(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 rioReward = multiplier.mul(rioPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 rioReward = multiplier.mul(rioPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        rio.mint(devaddr, rioReward.div(10));
        rio.mint(address(this), rioReward);
    ...
    function safeRioTransfer(address _to, uint256 _amount) internal {
        uint256 rioBal = rio.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > rioBal) {
            transferSuccess = rio.transfer(_to, rioBal);
        } else {
            transferSuccess = rio.transfer(_to, _amount);
        }
        require(transferSuccess, "safeRioTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _rioPerBlock) public onlyOwner {
        massUpdatePools();
        rioPerBlock = _rioPerBlock;
        emit UpdateEmissionRate(msg.sender, _rioPerBlock);
    }
}
